...
export const sendNotificationToUser = async (
    ...
    try {
        console.log(`Sending targeted notification to ${userId}: ${title}`);

        const userTokenDoc = await firestore.collection('fcmTokens').doc(userId).get();
        if (!userTokenDoc.exists || !userTokenDoc.data()?.token) {
            console.warn(`No FCM token found for user ${userId}`);
            return;
        }

        // In a real production environment, you would call a Cloud Function here
        // to send the FCM message using the Firebase Admin SDK.
        // For this demo/client-side implementation, we simulate it by adding to notifications collection
        // which the recipient might be subscribed to.
